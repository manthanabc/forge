...
> ToolExecutor<S>
{
    ...
    async fn call_internal(&self, input: Tools, tasks: &mut TaskList) -> anyhow::Result<Operation> {
        Ok(match input {
            ...
            Tools::ForgeToolAttemptPartialCompletion(_input) => {
                crate::operation::Operation::AttemptPartialCompletion
            }
    ...
    pub async fn execute(
        ...
        Ok(execution_result.into_tool_output(tool_name, truncation_path, &env, Some(context.session_metrics)))
    }
}
